...
test('should be able to import a stylesheet twice', checkFixture, 'duplicates', {
...
test('should error when file not found', t => {
    ...
    return postcss().use(atImport()).process(readFileSync(file), { from: file }).catch(err => t.truthy(err))
})
...
test('should contain a correct sourcemap', t => {
    return postcss()
        ...
        .then(result => {
            t.is(result.map.toString(), readFileSync('test/sourcemap/out.css.map', 'utf8').trim())
        })
})
...
test('should work with empty files', checkFixture, 'empty-and-useless', { path: 'test/fixtures/imports' }, null, [
