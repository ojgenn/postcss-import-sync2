...
test('should warn when not @charset and not @import statement before', t => {
    return Promise.all([
        ...
        results.forEach(result => {
            ...
            t.is(warnings[0].text, '@import must precede all other statements (besides @charset)')
        })
    })
})
      ...
      `
        ...
        .then(result => {
            ...
            result.warnings().forEach(warning => {
                t.is(warning.text, '@import must precede all other statements (besides @charset)')
            })
        })
})
